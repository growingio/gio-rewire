...
function compileLess(inputStream) {
  const mergeStylePath = path.join(getProjectPath(), `${pkgConfig.name}.less`)
  ...
  return merge2(
    inputStream.pipe(lessParseStream).pipe(gulp.dest('es')).pipe(through2.obj(function (_0, _1, next) { next() })),
    inputStream.pipe(lessMergeStream),
    gulp.src(`./${pkgConfig.name}.less`)
