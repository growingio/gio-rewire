...
const styleSource = ["src/**/*.less", "!src/**/*.stories.less"];

function usePlumber(input, options) {
    return gulp.src(input, options).pipe(
        plumber({
...
function compile2Lib(watch) {
    ...
    return merge2([
        ...
        usePlumber(["src/**/*.@(png|svg|json)"]).pipe(gulp.dest(esDir)),
...
function watch() {
    libCompile(() => {
        ...
        const watcher = gulp.watch([].concat(tsSource, styleSource), {
            ignoreInitial: true,
            delay: 100,
            pwd: true,
        });
...
gulp.task("lib-compile-watch", gulp.series([watch]));
